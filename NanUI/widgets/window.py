...
class CaptionButton(QPushButton):
    ...
    def __init__(
        self, icon_type: str = "minimize", parent: Optional[QWidget] = None
    ) -> None:
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        if is_pressed:
            bg = QColor(
                get_color("caption_close_pressed" if is_close else "caption_pressed")
            )
        elif is_hover:
            bg = QColor(
                get_color("caption_close_hover" if is_close else "caption_hover")
            )
        else:
...
class Window(QWidget):
    ...
    _resize_direction: Optional[str]
    _squared_corners: Optional[bool]
    ...
    def __init__(
        ...
        font = get_font(size=12, weight=QFont.Weight.Normal)
        # font.setWeight(QFont.Weight.Normal)  # 常规字重，不粗
    ...
    def event(self, event: QEvent) -> bool:
        if self._resizable:
            ...
            if event_type == QEvent.Type.HoverMove:
                hover_event = cast(QHoverEvent, event)
            ...
            # MouseButtonPress：开始调整大小（整个窗口边缘均可触发）
                ...
                if mouse_event.button() == Qt.MouseButton.LeftButton:
                    ...
                    if direction is not None:
                        ...
                        self._resize_start_pos = mouse_event.globalPosition().toPoint()
            ...
            # MouseMove：执行调整大小（仅按键拖拽时）
                ...
                if self._resize_direction is not None:
                    self._performResize(cast(QMouseEvent, event))
                    return True
            ...
            # MouseButtonRelease：结束调整大小
                ...
                if (
                    ...
                    and self._resize_direction is not None
                ):
                    self._resize_direction = None
                    self._resize_start_pos = None
                    self._resize_start_geo = None
    ...
    # ---------- 辅助方法（调整大小相关） ----------
    def _getResizeDirection(self, pos: QPoint) -> Optional[str]:
    ...
    def _setCursorShape(self, direction: str) -> None:
        shapes = {
        ...
        cursor = shapes.get(direction, Qt.CursorShape.ArrowCursor)
        self.setCursor(cursor)

    def _performResize(self, event: QMouseEvent) -> None:
    ...
    def _update_appearance(self) -> None:
        ...
        if self._squared_corners != maximized:
            self._squared_corners = maximized
            self._apply_corner_style(maximized)

    def _apply_corner_style(self, squared: bool) -> None:
        """
